...
int main(int argc, const char *argv[])
{
    ...
    while (true)
    {
        ...
        comboStat.combination = detectorType + " " + descriptorType + " " + matcherType + " " + descriptorCategory + " " + selectorType;
        ...
        for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex++)
        {
            ...
            if (bFocusOnVehicle)
            {
                ...
                std::vector<cv::KeyPoint> keepPoints;
                for (auto& pt : keypoints)
                {
                    if (vehicleRect.contains(pt.pt))
                    {
                        keepPoints.push_back(cv::KeyPoint(pt));
                    }
                }
                keypoints = keepPoints;
            ...
            executionTime += descKeypoints((dataBuffer.end() - 1)->keypoints, (dataBuffer.end() - 1)->cameraImg, descriptors, descriptorType);
